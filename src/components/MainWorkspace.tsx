...
import { SelfPanelView } from './SelfPanelView';
import { Hash, Code2, KeySquare, X, Columns, Maximize2 } from 'lucide-react';
...
export function MainWorkspace({ 
  ...
  const getIcon = (type: AppTab['type']) => {
    switch (type) {
      ...
      case 'self': return <KeySquare className="w-4 h-4 mr-2 text-gray-500" />;
      default: return null;
    }
  };
  ...
  const renderContent = (tab: AppTab) => {
    switch (tab.type) {
      ...
      case 'self': return <SelfPanelView />;
      default: return <div className="p-8 text-gray-500">[{tab.label} view under construction]</div>;
    }
  };
      ...
      <div className="h-10 shrink-0 bg-[#0F1115] border-b border-gray-800 flex items-center justify-between pr-2">
        ...
        <div className="flex space-x-2 px-2 shrink-0">
          <button 
            ...
            title="Toggle Split View (Code + Chat)"
